...
// 기존 지출 행에 영수증 파일 첨부(미첨부/누락 보완) — Drive 업로드 후 drive:<fileId> 저장
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const teamRow = (await db.select({ name: schema.teams.name }).from(schema.teams).where(eq(schema.teams.id, e.teamId)).limit(1))[0];
